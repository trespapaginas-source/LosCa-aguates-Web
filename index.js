(function () {
  ...
  const updateCalendarOutputs = () => {
    ...
    const checkinFieldLabel = document.querySelector('#booking-field-checkin .booking-label');
    ...
    // Mostrar/ocultar fecha de salida (Salida) en la barra de reservas superior y ajustar columnas
    if (selectedPlan === 'Pasadía') {
      if (checkoutField) checkoutField.style.display = 'none';
      if (checkinFieldLabel) checkinFieldLabel.textContent = 'Día del Pasadía';
      if (bookingBar) bookingBar.classList.add('booking-bar--compact');
      if (selectionStart) selectionEnd = selectionStart; // Forzar fin de rango igual a inicio
    } else if (selectedPlan === 'Eventos') {
      if (checkoutField) checkoutField.style.display = 'none';
      if (checkinFieldLabel) checkinFieldLabel.textContent = 'Día del Evento';
      if (bookingBar) bookingBar.classList.add('booking-bar--compact');
      if (selectionStart) selectionEnd = selectionStart;
    } else {
      if (checkoutField) checkoutField.style.display = 'flex';
      if (checkinFieldLabel) checkinFieldLabel.textContent = 'Llegada';
      if (bookingBar) bookingBar.classList.remove('booking-bar--compact');
    }
      ...
      for (let i = 1; i <= lastDay; i++) {
        ...
        if (cellDate.getTime() < todayDate.getTime()) {
          ...
          dayEl.addEventListener('click', (e) => {
            e.preventDefault();
            if (selectedPlan === 'Pasadía' || selectedPlan === 'Eventos') {
              // El pasadía y evento es de un único día
              selectionStart = cellDate;
              selectionEnd = cellDate;
              // Desplazar suavemente al botón de WhatsApp
              setTimeout(() => {
                const btnCotizar = document.getElementById('btn-cotizar');
                if (btnCotizar) {
                  btnCotizar.scrollIntoView({ behavior: 'smooth', block: 'center' });
                }
              }, 100);
            } else {
  ...
  function initApp() {
    ...
    if (checkinInput) {
      checkinInput.addEventListener('change', (e) => {
        if (e.target.value) {
          ...
          if (selectedPlan === 'Pasadía' || selectedPlan === 'Eventos') {
            selectionEnd = selectionStart;
            // Desplazar al botón de envío o hacer focus al submit
            setTimeout(() => {
    ...
    const expressFieldCheckout = document.getElementById('express-field-checkout');
    const expressDateCheckoutInput = document.getElementById('express-date-checkout');
    const expressDateLabel = document.getElementById('express-date-label');
    ...
    if (expressFab && expressCard && expressClose) {
      ...
      // Al cambiar la fecha de llegada, establecer la fecha mínima para la de salida y auto-enfocar el siguiente paso
      if (expressDateInput && expressDateCheckoutInput) {
        expressDateInput.addEventListener('change', () => {
          if (expressDateInput.value) {
            expressDateCheckoutInput.min = expressDateInput.value;
            // Si la fecha de salida es anterior a la llegada, limpiarla
            if (expressDateCheckoutInput.value && expressDateCheckoutInput.value < expressDateInput.value) {
              expressDateCheckoutInput.value = '';
            }
            // Si es un plan de un solo día (Pasadía o Evento), enfocar el submit directo
            if (expressPlan === 'Pasadía' || expressPlan === 'Eventos') {
              setTimeout(() => {
                const submitBtn = expressCard.querySelector('.express-submit-btn');
                if (submitBtn) submitBtn.focus();
              }, 100);
            } else {
              // Si es hospedaje, enfocar el campo de salida
              setTimeout(() => {
                expressDateCheckoutInput.focus();
                if (typeof expressDateCheckoutInput.showPicker === 'function') {
                  expressDateCheckoutInput.showPicker();
                }
              }, 100);
            }
          }
        ...
        expressDateCheckoutInput.addEventListener('change', () => {
          if (expressDateCheckoutInput.value) {
            setTimeout(() => {
              const submitBtn = expressCard.querySelector('.express-submit-btn');
              if (submitBtn) submitBtn.focus();
            }, 100);
      ...
      expressPlanBtns.forEach(btn => {
        btn.addEventListener('click', () => {
          ...
          if (expressPlan === 'Hospedaje') {
            if (expressFieldCheckout) expressFieldCheckout.style.display = 'flex';
            if (expressDateCheckoutInput) expressDateCheckoutInput.required = true;
            if (expressDateLabel) expressDateLabel.textContent = '¿Cuándo llegas? (Llegada)';
          } else {
            if (expressFieldCheckout) expressFieldCheckout.style.display = 'none';
            if (expressDateCheckoutInput) {
              expressDateCheckoutInput.required = false;
              expressDateCheckoutInput.value = '';
            }
            if (expressDateLabel) {
              expressDateLabel.textContent = (expressPlan === 'Eventos') ? '¿Qué día es el evento?' : '¿Qué día vienes?';
            }
          }
        });
      });
      ...
      if (expressForm) {
        expressForm.addEventListener('submit', (e) => {
          ...
          let formattedDateText = `${dayName} ${selectedDate.getDate()} de ${monthName} de ${selectedDate.getFullYear()}`;
          ...
          if (expressPlan === 'Hospedaje') {
            const rawCheckoutDate = expressDateCheckoutInput ? expressDateCheckoutInput.value : '';
            if (rawCheckoutDate) {
              const checkoutParts = rawCheckoutDate.split('-');
              const checkoutDate = new Date(checkoutParts[0], checkoutParts[1] - 1, checkoutParts[2]);
              const checkoutDayName = ["Domingo", "Lunes", "Martes", "Miércoles", "Jueves", "Viernes", "Sábado"][checkoutDate.getDay()];
              const checkoutMonthName = ["Enero", "Febrero", "Marzo", "Abril", "Mayo", "Junio", "Julio", "Agosto", "Septiembre", "Octubre", "Noviembre", "Diciembre"][checkoutDate.getMonth()];
              const formattedCheckoutText = `${checkoutDayName} ${checkoutDate.getDate()} de ${checkoutMonthName} de ${checkoutDate.getFullYear()}`;
              
              formattedDateText = `${formattedDateText} hasta el ${formattedCheckoutText}`;

              // Calcular noches y presupuesto
              const diffTime = Math.abs(checkoutDate.getTime() - selectedDate.getTime());
              let nights = Math.ceil(diffTime / (1000 * 60 * 60 * 24));
              if (nights < 1) nights = 1;
              const total = numGuests * 75000 * nights;
              budgetText = `\n💰 Valor Estimado: $${total.toLocaleString('es-CO')} COP ($75.000 por persona por noche · ${nights} ${nights === 1 ? 'noche' : 'noches'})`;
            } else {
              const total = numGuests * 75000;
              budgetText = `\n💰 Valor Estimado: $${total.toLocaleString('es-CO')} COP ($75.000 por persona / noche)`;
            }
          } else if (expressPlan === 'Pasadía') {
            const total = numGuests * 35000;
            budgetText = `\n💰 Valor Estimado: $${total.toLocaleString('es-CO')} COP ($35.000 por persona)`;
          }

          const message = `¡Hola Cabaña Los Cañaguates! 🌴✨\nMe gustaría cotizar un servicio rápido con los siguientes detalles:\n\n📅 Plan: ${expressPlan}\n📆 Fechas: ${formattedDateText}\n👥 Cantidad de Personas: ${numGuests} personas${budgetText}\n\nQuedo atento a la disponibilidad. ¡Muchas gracias!`;
